...
void initialize_graphics_mode() {
    ...
    initgraph(&gd, &gm, "C:\\TURBOC3\\BGI");  // location where the file is
                                              // going to be saved in your PC
    ...
    settextjustify(CENTER_TEXT,
                   CENTER_TEXT);  // TO MAKE TEXT IN CENTER OF SCREEN
    setbkcolor(WHITE);
...
void get_input() {
    ...
    while (1) {
        ...
        if (x >= (left - 3) && y >= (top - 3) && y <= (top + 30 + 3) &&
            x < left) {
            draw_bar();
            left = left + 4;

            if (left > 350) left = 190;
        ...
        else if (x <= (left + 100 + 3) && y >= (top - 3) &&
                 y <= (top + 30 + 3) && x > (left + 100)) {
            draw_bar();
            left = left - 4;

            if (left < 190) left = 350;
        ...
        else if (x > (left - 3) && y >= (top - 3) && y < (top) &&
                 x <= (left + 100 + 3)) {
            draw_bar();
            top = top + 4;
        ...
        else if (x > (left - 3) && y > (top + 30) && y <= (top + 30 + 3) &&
                 x <= (left + 100 + 3)) {
            draw_bar();
            top = top - 4;
