...
bool compare(Edge a, Edge b) {
    return a.weight <
           b.weight;  // return true if weight of edge a is less than edge b
}
...
void unionSets(int a, int b, vector<int>& parent, vector<int>& rank) {
    ...
    if (a != b) {            // if a and b are in different sets
        // Union by rank: attach the tree with a smaller rank under the root of
        // the tree with a larger rank
        if (rank[a] < rank[b])
...
// Function implementing Kruskal's algorithm to find the weight of the Minimum
// Spanning Tree (MST)
    ...
    for (Edge e : edges) {
        ...
        if (findSet(e.u, parent) != findSet(e.v, parent)) {
            ...
            unionSets(e.u, e.v, parent,
                      rank);  // unite the sets containing u and v
        }
    }
