...
void topologicalSortUtil(int node, vector<vector<int>>& adj,
    ...
    for (int neighbor : adj[node]) {
        if (!visited[neighbor]) {
            topologicalSortUtil(neighbor, adj, visited,
                                Stack);  // Recur for unvisited neighbors
        }
    }

    Stack.push(node);  // Push the current node to the stack after visiting all
                       // its neighbors
}
...
/*
Explanation:
- This function implements topological sorting for a directed acyclic graph
(DAG).
- Topological sorting orders the nodes such that for every directed edge (u, v),
...
- After finishing all nodes, it prints the nodes in topological order by popping
them from the stack.
*/
