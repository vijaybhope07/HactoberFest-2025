...
// Function to implement Kruskal's algorithm for finding Minimum Spanning Tree
// (MST)
int kruskal(vector<Edge>& edges, int V) {
    ...
    for (Edge edge : edges) {
        if (find(edge.u, parent) !=
            find(edge.v, parent)) {  // If u and v are not connected
...
(MST) of a graph.
- The MST connects all vertices with the minimum total edge weight without
forming any cycles.
- It uses a Union-Find data structure to manage and merge disjoint sets of
nodes.
- The edges are sorted by weight to ensure that the smallest edges are
considered first.
