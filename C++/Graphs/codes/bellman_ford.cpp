...
bool bellmanFord(int start, int V, vector<vector<int>>& edges,
                 vector<int>& dist) {
...
/*
Explanation:
- This function implements the Bellman-Ford algorithm to find the shortest paths
from a starting node to all other nodes in a graph.
- Unlike Dijkstra's, it can handle negative weights and detects negative cycles.
- The algorithm relaxes all edges multiple times (V-1 times, where V is the
number of vertices).
- After relaxation, it checks for negative cycles by attempting to relax the
edges one more time.
