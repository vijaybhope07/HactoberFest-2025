...
/*
Explanation:
- This function implements Dijkstra's algorithm to find the shortest path from a
starting node to all other nodes in a weighted graph.
- It uses a priority queue (min-heap) to process the next node with the smallest
known distance.
- The 'dist' array keeps track of the shortest distance from the start node to
every other node.
- The algorithm iteratively updates the distances to each neighboring node based
on the current node's distance.
- It returns an array of the shortest distances to all nodes from the start
node.
*/
