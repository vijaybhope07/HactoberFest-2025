...
- It starts from a specified node (the 'start' node) and explores all of its
neighboring nodes first.
- It uses a queue to keep track of nodes that need to be visited, ensuring that
nodes are processed in the order they are discovered.
- A 'visited' array is used to mark nodes as visited to prevent re-visiting
them, which would lead to an infinite loop.
