...
void graph::prims_algorithm() {
    ...
    while (1) {
        int visited_flag = 0;
        for (i = 0; i < v; i++) {
            if (visited[i] == 1) visited_flag++;
        }

        if (visited_flag == v) break;
        edge_ctr = 0;
        for (i = 0; i < v; i++) {
            if (visited[i] == 1) {
                for (j = 0; j < v; j++) {
                    if (data[i][j] != 999) {
                        // before adding that edge in ed array check whether it
                        // has already been added in MST array

                        mst_flag = 0;
                        for (int k = 0; k < mst_ctr; k++) {
                            if ((mst[k].v1 == i && mst[k].v2 == j) ||
                                (mst[k].v1 == j && mst[k].v2 == i)) {
                                mst_flag = 1;
                                break;
                            }
                        }
                        ...
                        for (int k = 0; k < discarded_ctr; k++) {
                            if ((discarded_edge[k].v1 == i &&
                                 discarded_edge[k].v2 == j) ||
                                (discarded_edge[k].v1 == j &&
                                 discarded_edge[k].v2 == i)) {
                                discarded_flag = 1;
                                break;
                            }
                        }
                        // edge is not in MST array and is not present in
                        // discarded array
...
int main() {
    ...
    for (int i = 0; i < v; i++) g.data[i][i] = 999;

    for (int i = 0; i < v; i++) {
        for (int j = i + 1; j < v; j++) {
            cout << "\n Enter the cost of edge between " << i + 1 << " to "
                 << j + 1 << " : ";
            cin >> g.data[i][j];
