...
int sumRange(vector<int> nums, int left, int right) {
    ...
    if (left == 0) {
        ...
        return pre[right] -
               pre[left - 1];  // calculates the range of sum required
    }
    return -1;
}
...
int main() {
    ...
    cout << "Answer 1: " << sumRange(nums, 0, 2)
         << endl;  // sum of [-2,0,3] = 1
    cout << "Answer 2: " << sumRange(nums, 2, 5)
         << endl;  // sum of [3,-5,2,-1] = -1
    cout << "Answer 3: " << sumRange(nums, 0, 5)
         << endl;  // sum of whole array = -3
