   ...
   public:
    ...
    bool insertFront(int value) {
        if (isFull()) {
            ...
            // If the deque is empty, set front and rear to 0 and insert the
            // value
            front = 0;
            rear = 0;
    ...
    bool deleteFront() {
        if (isEmpty()) {
            ...
            front =
                (front - 1 + maxSize) % maxSize;  // Wrap around if necessary
            return true;                          // Deletion successful
        }
    }
    ...
    bool isFull() {
        return (front + 1) % maxSize ==
               rear;  // Return true if next front position equals rear
    }
};
