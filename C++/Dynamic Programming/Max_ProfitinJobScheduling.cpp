...
3)go through sorted jobs, find the best-fit end time(previous of upper bound of
current starting time) from dp, and add current job profit to that one if it is
greater than the maximum profit that has been present in dp, then add that
calculated profit to the dp. 4)maximum profit has been stored at the end of dp,
thus return (last element)->second.
*/
class Solution {
   public:
