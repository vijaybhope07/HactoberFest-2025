...
void Merge(int A[], int l, int mid,
           int h)  // this is for two array inside one array
{
    ...
    for (i = l; i <= h;
         i++)  // this is for copy all the elements from B to A( back )
        A[i] = B[i];
}
void IMergeSort(int A[], int n) {
    int p, l, h, mid, i;
    for (p = 2; p <= n;
         p = p * 2)  // for passes // always list size will be doubled
    {
        for (i = 0; i + p - 1 < n;
             i = i + p)  // for the sub lsit // merging all list in a single
                         // pass // i sud incremented by i  + p
        {
    ...
    if (p / 2 < n)  // if there are some more elements remaning then we sud
                    // merge it as it is
    {
