...
def shorten():
    ...
    # Generate base 62 code with a random number
...
def redirectUrl(code):
    ...
    if code in url_map:
        # will redirect with the long url using short url
...
@app.route("/")
def home():
    return jsonify({"message": "Tiny Url Shortner API", "routes": [
                   "shorten(POST)", "/r/<code> (GET)"]})
