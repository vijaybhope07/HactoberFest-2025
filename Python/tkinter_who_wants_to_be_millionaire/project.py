...
class MillionaireGame:
    ...
    def setup_ui(self):
        ...
        self.title_label = tk.Label(
            self.main_frame, text="WHO WANTS TO BE A MILLIONAIRE?", font=(
                'Arial', 24, 'bold'), fg='#FFD700', bg='#000033')
        ...
        self.winnings_label = tk.Label(
            self.game_frame, text="Current Winnings: $0", font=(
                'Arial', 16, 'bold'), fg='#00FF00', bg='#000033')
        self.winnings_label.pack(pady=10)

        self.question_frame = tk.Frame(
            self.game_frame, bg='#000066', relief='raised', bd=3)
...
Good Luck! 🍀
    ...
    def update_winnings_display(self):
        self.winnings_label.configure(
            text=f"Current Winnings: {
                format_currency(
                    self.winning_amount)}")
    ...
    def submit_answer(self):
        ...
        if validation_result is None:
            self.question_label.configure(
                text="Please select an answer!", fg='#FF0000')
            return
        ...
        if validation_result:
            self.winning_amount = calculate_winnings(
                self.current_question, True, self.winning_amount)
            self.question_label.configure(
                text=f"Correct! You've won {
                    format_currency(
                        self.winning_amount)}!",
                fg='#00FF00')
            ...
            self.winning_amount = calculate_winnings(
                self.current_question, False, self.winning_amount)
    ...
    def ask_continue(self):
        if self.current_question > 1:
            self.question_label.configure(
                text=f"You have {
                    format_currency(
                        self.winning_amount)}. Continue to next question?",
                fg='#FFD700')
    ...
    def quit_game(self):
        self.question_label.configure(
            text=f"Are you sure you want to quit with {
                format_currency(
                    self.winning_amount)}?",
            fg='#FFD700')
    ...
    def show_final_result(self, won_all):
        ...
        if won_all:
            ...
            message_text = f"Thank you for playing!\nYou've answered {
                self.current_question} questions correctly.\nYou're taking home {
                format_currency(
                    self.winning_amount)}!"
            title_color = '#FF6600'
